...
async function extractOrderItems(menu: MenuItem[], cart: OrderItem[], userMessage: string): Promise<ParsedOrder | null> {
  const cartText =
    cart.length > 0
      ? cart.map((item) => `${item.qty} × ${item.name}`).join("\n")
      : "(пока пусто)";
...
Текущая корзина клиента ДО этого сообщения (только для расчёта разницы у позиций, которые клиент называет по имени в этом сообщении — НЕ повод трогать позиции, которые он не называет):
${cartText}

Правила:
- В поле "items" — позиции, которые нужно ДОБАВИТЬ. В поле "removeItems" — которые нужно УБРАТЬ или уменьшить.
- В обоих полях используй ТОЧНОЕ название позиции из меню выше (символ в символ).
- ЖЕЛЕЗНОЕ ПРАВИЛО: в items и removeItems может попасть ТОЛЬКО та позиция, чьё название (или явный синоним/тип блюда) реально присутствует как слово в сообщении клиента ниже. Если клиент не написал слово, связанное с какой-то позицией, эта позиция НЕ появляется НИ в items, НИ в removeItems — даже если она есть в текущей корзине и даже если в корзине "тоже надо бы поправить количество". Одно упомянутое блюдо не может влиять на другие, не упомянутые.
- Если клиент называет АБСОЛЮТНОЕ количество для конкретной названной позиции, которая уже есть в корзине ("только один донер", "пусть будет 2 шаурмы", "хватит одного X") — сравни с текущим количеством ИМЕННО ЭТОЙ позиции в корзине выше и посчитай разницу для НЕЁ ОДНОЙ. Если в корзине несколько РАЗНЫХ позиций, подходящих под общее слово (например разные виды донера), и не очевидно, какую именно клиент имеет в виду, — не трогай корзину и поставь "unclear": true.
- Будь ОСОБЕННО внимателен к виду мяса — "говяжий", "куриный" и "ассорти" это РАЗНЫЕ позиции, не путай их между собой.
- Убрать позицию можно любой естественной фразой: "уберите X", "без X", "не надо X", "минус X", "X не нужен" — все эти формулировки означают removeItems.
...
Сообщение: "без шаурмы"
Ответ: {"items": [], "removeItems": [{"name": "Шаурма куриная"}], "finished": false, "unclear": false}
Пример (показывает правило "не трогай неупомянутое"): корзина = "2 × Донер куриный в лаваше, 1 × Шаурма куриная", сообщение = "только один донер куриный" → items: [], removeItems: [{"name": "Донер куриный в лаваше", "qty": 1}] — шаурму НЕ трогаем, её не назвали.
...
export async function processOrderMessage(sub: SubscriptionRow, message: IncomingMessage) {
  ...
  if (order.state === "collecting") {
    ...
    const parsed = await extractOrderItems(menu, order.items, text);
    if (!parsed) {
      await reply(sub, message.chatId, t.fallback());
      return;
    }
